...
import { addMockBooking, addMockPayment } from '../../data/mockData'
    ...
    try {
      const bookings = cart.map(item => {
        return addMockBooking({
          ...
          eventImage: item.urlImage || '',
        })
      })

      const payments = bookings.map((b, i) => {
        const it = cart[i]
        return addMockPayment({
          bookingId: b.bookingId, userId: user.idUser, eventId: it.idEvent || it.id,
          ticketQuantity: it.qty || 1, totalPrice: (it.price || 0) * (it.qty || 1),
          paymentMethod: method, amount: (it.price || 0) * (it.qty || 1),
          status: 'APPROVED', paymentStatus: 'APPROVED',
        })
      })

      const fp = payments[0]
      setConfirmedData({ cartSnapshot: [...cart], bookings, paymentId: fp.paymentId })
  ...
  const handleDownloadTickets = async () => {
    ...
    try {
      ...
      for (let i = 0; i < cartSnapshot.length; i++) {
        ...
        for (let j = 0; j < qty; j++) {
          expandedTickets.push({ ...item, qty: 1, price: item.price })
        }
      }
        ...
        {step !== 3 && (
          <div className="chk-steps">
            ...
            <div className="chk-step">
              ...
              <div className="chk-step-label">Confirmación</div>
            </div>
          </div>
        )}
